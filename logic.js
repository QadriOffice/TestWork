...
function _om(rowIdx, type) {
  ...
  setTimeout(() => {
    if(!mbMap) {
      mbMap = L.map('map').setView([20.5937, 78.9629], 5);
...
function _pm(lng,lat) {
  if(mbMarker) mbMap.removeLayer(mbMarker);
  mbMarker = L.marker([lat,lng], {
    icon: L.divIcon({
      className:'',
      html:'<div style="background:#cc0000;width:14px;height:14px;border-radius:50%;border:2px solid #fff;box-shadow:0 1px 4px rgba(0,0,0,0.4);"></div>',
      iconAnchor:[7,7]
    })
  }).addTo(mbMap);
  mbMap.flyTo([lat,lng], 13);
}
...
// Global aliases — HTML onclick attributes ke liye
window.openModal          = _om;
window.closeModal         = _cm;
window.confirmLoc         = _cfl;
window.doSearch           = _ds;
window.setSpecial         = _ss;
window.clearAllData       = _cad;
window.sendViaEmail       = _sve;
window.buildTableStructure = _bts;
}(window,document));